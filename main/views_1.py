    ...
    if request.method == 'POST':
        ...
        if request.session['time'] == 'am8':
            ...
            if form.is_valid():
                ...
                #     comment.message_response = '오늘은 즐거운 하루였네요'
                # else:
                #     comment.message_response = '힘내세요!'
                # comment.save()
            ...
            if form.is_valid():
                ...
                if request.session['sbuck'] == 'americano':
                    ...
                    if any(x in comment.message for x in ['tall', '톨']):
                        comment.message_response = '아이스 아메리카노 톨 사이즈로 주문합니다. 4100원입니다.'
                        money.money -= 4100
                    elif any(x in comment.message for x in ['grande', '그란데']):
                        comment.message_response = '아이스 아메리카노 그란데 사이즈로 주문합니다. 4600원입니다.'
                        money.money -= 4600
                    money.save()
                    comment.save()
                    ...
                    money = Money.objects.latest('pk')
                    if any(x in comment.message for x in ['tall', '톨']):
                        comment.message_response = '프라푸치노 톨 사이즈로 주문합니다. 5600원입니다.'
                        money.money -= 5600
                    elif any(x in comment.message for x in ['grande', '그란데']):
                        comment.message_response = '아이스 아메리카노 그란데 사이즈로 주문합니다. 6100원입니다.'
                        money.money -= 6100
                    money.save()
                    comment.save()
                else:
